codeunit 50107 "Certification Notifier"
{
    ...
    begin
        ...
        if not Setup."Email Notifications Enabled" then
            exit;

        if EmployeeCertification.Status <> EmployeeCertification.Status::"Expiring Soon" then
            exit;

        if not Employee.Get(EmployeeCertification."Employee No.") then
            exit;

        Message('Notification: %1''s certification "%2" expires on %3',
            Employee.FullName(), EmployeeCertification."Certification Name", EmployeeCertification."Expiry Date");
    end;
}